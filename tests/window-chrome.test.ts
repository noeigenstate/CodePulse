...
test('default HUD renders a project-first single lane with an optional compact grid', () => {
  ...
  const groupRule = css.match(/^\s*\.project-deck-group \{([\s\S]*?)^\s*\}/m)?.[1]

  assert.match(appSource, /buildProjectDeckGroups/)
  assert.match(appSource, /showUsageStrip/)
  assert.match(appSource, /setHudContentHeight/)
  assert.match(appSource, /formatRelative\(card\.updatedAt, now, locale\)/)
  assert.match(appSource, /onAck\(card\.agentType, group\.workspacePath\)/)
  assert.doesNotMatch(appSource, /formatRelative\(group\.updatedAt, now, locale\)/)
  ...
  assert.ok(groupRule)
  assert.match(groupRule, /grid-template-columns: minmax\(0, 1fr\)/)
  assert.doesNotMatch(groupRule, /grid-column:\s*1\s*\/\s*-1/)
