...
| 🧭 **多 agent、多工作区** | 跨项目并发的 Codex、Claude Code、Grok 与 Kimi Code 会话，各自独立追踪。                                 |
...
| 📈 **上下文追踪**         | 展示四种 CLI 的紧凑上下文窗口信息，优先使用 CLI 提供的精确数据。                                        |
| 🎟️ **配额感知**           | Claude / Kimi：5 小时 + 每周；Codex / Grok：仅每周额度，并匹配 CLI 模型或配额桶。                       |
