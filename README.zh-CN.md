...
AI 编程代理很擅长无人值守地干活，却不擅长在需要你时提示你。CodePulse
监听 Codex、Claude Code、Grok 与 Kimi Code 暴露的生命周期 hook，把每一个事件
送进同一个状态机，再把结果投射到常驻的副驾 HUD：

- 🪟 **HUD-first 桌面表面** —— 透明、无边框、置顶，优先定位在副屏；需要提示时
  被动显现，不抢走当前窗口焦点。
- 🟠 **一种注意语言** —— 日常工作保持中性；需要留意或介入时，整个项目块变为橙色。
  全局注意带会让这些项目在「后台」打开、详细分栏被隐藏或滚出屏幕时仍然可见。不再生成
  系统通知或临时 toast。
- 📈 **用量继续常驻** —— 项目块保留上下文与额度；右上角「后台」仍可查看 Token、
  时长、项目、模型占比和高峰时段等本机 SQLite 汇总。
- ⚙️ **HUD 显示设置** —— 可调整面板透明度、自动/白色/黑色外观、显示哪些 CLI，
  以及橙色提示使用常亮、呼吸或更强脉冲。
- ◐ **克制的托盘提示** —— 普通活动为中性；只有需要注意时才使用同一种橙色。
...
| 🪟 **自适应分屏**         | 仅在对应 CLI 有任务或仍保留额度时显示分栏，可从一栏自动扩展到四栏。                                     |
| 🪟 **HUD-first 外壳**     | 透明、无边框、置顶并优先进入副屏；提示状态被动显现，不夺取当前窗口焦点。                                |
...
| 🟠 **单一橙色提示**       | 日常活动保持中性；未读完成、明确的输入/授权请求和未读故障才提升为橙色。                                 |
...
5. 运行一轮 Claude Code、Codex、Grok 或 Kimi Code 任务。Dashboard 只显示有活动的 CLI
   对应分屏（自适应布局）。
6. 点击齿轮可设置 HUD 透明度、自动/白色/黑色外观、橙色提示动效，
   并增删要显示的 CLI 面板。
