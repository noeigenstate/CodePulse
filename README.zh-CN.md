...
- 📊 **实时 Dashboard** —— 按 agent、按工作区的卡片，展示状态、模型、
  本轮耗时、上下文用量与配额。
...
| 📈 **上下文追踪**         | 展示 Claude 与 Codex 的紧凑上下文窗口信息，优先使用 CLI 提供的精确数据。                                |
...
1. 打开 CodePulse。
2. CodePulse 会检查本机 Claude Code 和 Codex CLI 配置。
...
4. 如果配置弹窗提示需要信任 Codex hook，打开一个 Codex 项目终端，运行 `/hooks`，
   ...
   - `PostToolUse`
   - `Stop`
5. 运行一轮 Claude Code 或 Codex 任务，Dashboard 就会开始同步。
...
在任一 agent 中跑一次任务，观察 Dashboard 亮起；也可以检查本地 API：
...
Claude / Codex hook 缺失。Codex 如果提示需要信任，请运行一次 `/hooks` 并信任
CodePulse hook。
